...
class ForagingEnvWrapper(gym.Wrapper):
    def __init__(self, env):
        super().__init__(env)

    def reset2(self, lbf_features=-1):
        if lbf_features != -1:
            self.env.field_size = (lbf_features["field_size"], lbf_features["field_size"])
            self.env.max_player_level = lbf_features["max_player_level"]
            self.env.max_food = lbf_features['f']
...
class _GymmaWrapper(MultiAgentEnv):
    def __init__(self, key, time_limit, pretrained_wrapper, seed, **kwargs):
        ...
        self._env = ForagingEnvWrapper(self._env)
    ...
    def reset(self, lbf_features=-1):
        """ Returns initial observations and states"""
        # print(lbf_features)
        self._obs = self._env.reset2(lbf_features)
        # print(self._env.field)
        self._obs = [
            np.pad(
                o,
                (0, self.longest_observation_space.shape[0] - len(o)),
